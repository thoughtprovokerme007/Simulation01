...
const pieData = [
  { name: 'Direct Medical', value: 55, color: '#3b82f6' },
  { name: 'Direct Non-Medical', value: 20, color: '#f59e0b' },
  { name: 'Indirect Costs', value: 25, color: '#10b981' },
];
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 md:p-10 space-y-10">
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl shadow-lg border border-slate-50">
          <div className="flex items-center justify-between mb-8">
            <div className="space-y-0.5">
              <h3 className="text-2xl font-black text-gray-900 tracking-tight">Economic Divergence</h3>
              <p className="text-sm text-gray-400 font-bold uppercase tracking-widest">10-Year Cost (₹Lacs)</p>
            </div>
        ...
        <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl flex flex-col items-center justify-center text-center space-y-6 overflow-hidden">
          <h3 className="text-lg font-black uppercase tracking-widest opacity-60">Distribution</h3>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {[
          { icon: <Stethoscope size={24} />, title: "Direct Medical", desc: "Drug escalation, acute events, and consults.", bg: "bg-red-50 text-red-600" },
          ...
          <div key={i} className={`p-8 rounded-3xl border border-slate-100 shadow-lg transition-transform hover:-translate-y-1 ${card.bg.includes('blue') ? 'bg-blue-600' : 'bg-white'}`}>
             <div className="flex flex-col gap-4">
                <div className={`p-3.5 rounded-2xl w-fit ${card.bg}`}>
                  {card.icon}
                </div>
                <div className="space-y-1">
                  <h4 className={`text-xl font-black tracking-tight ${card.bg.includes('white') ? 'text-white' : 'text-gray-900'}`}>{card.title}</h4>
                  <p className={`text-sm leading-relaxed ${card.bg.includes('white') ? 'text-white/80' : 'text-gray-500 font-medium'}`}>{card.desc}</p>
                </div>
             </div>
          </div>
        ))}
