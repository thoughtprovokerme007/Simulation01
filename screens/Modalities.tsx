...
const Modalities: React.FC<ModalitiesProps> = ({ onOpenSource }) => {
  ...
  const icons: Record<Modality, React.ReactNode> = {
    Rybelsus: <Box size={32} className="text-white" />,
    Ozempic: <Zap size={32} className="text-white" />,
    Wegovy: <Target size={32} className="text-white" />,
  };

  // Novo Nordisk inspired palette
  // Rybelsus: Primary Corporate Navy
  // Ozempic: Secondary Blue/Cyan tones
  // Wegovy: Teal/Green for weight management distinction
  const colors: Record<Modality, string> = {
    Rybelsus: 'from-blue-600 to-blue-900',
    Ozempic: 'from-cyan-600 to-blue-700',
    Wegovy: 'from-teal-600 to-teal-800',
  };
          ...
          <button
            ...
            className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-lg transition-all border-2 ${
              selectedModality === m.id
                ? 'bg-white border-blue-600 text-blue-900 shadow-lg scale-105'
        ...
        <div className="p-10 grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-2xl font-black text-blue-900 tracking-tight">Baseline</h3>
            </div>
            <div className="grid gap-4">
              {active.withoutCKLM.map((row, idx) => (
                <div key={idx} className="p-6 rounded-2xl bg-slate-50 border border-slate-100 space-y-1 hover:bg-white hover:shadow-lg transition-all">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{row.row}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4">
              {active.withCKLM.map((row, idx) => (
                <div key={idx} className="p-6 rounded-2xl bg-blue-50/50 border border-blue-100 space-y-1 hover:bg-white hover:shadow-lg transition-all">
